...
```
cargo install --git git@github.com:samugi/stylus.git stylus-cli --force
```
...
no schema lookup, no picking functions:

- **Enable everything**: `stylus-cli encode --all`, then put the result in the config (step 5).
- **Disable everything**: set the config field to an empty string (step 5) — no blob needed.
