...
/// Replace the active set from a string produced by [`encode`]: exactly the
/// decoded ids are enabled, everything else is disabled.
...
/// Replace the *child-only* set from a string produced by [`encode`]: exactly
/// the decoded ids are put in child-only mode, every other function is made
/// root-capable again.
///
...
/// orthogonal to [`set_enabled_encoded`]: a function must be enabled to trace
/// at all, and being child-only additionally suppresses it when it would
